export const esp32FirmwareV13Code = `/*
  =============================================================================
  ...
  // 3. PŘÍKAZ K ULOŽENÍ KONFIGURACE DO NVS (config_relay)
    ...
    if (index >= 0 && index < 8) {
      ...
      if (doc["power_on"].is<String>()) {
        ...
        else relays[index].powerOnDefault = 0;
      }
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), mqtt_user, mqtt_pass)) {
      ...
      // Okamžité odeslání aktuální reality relé a senzorů
      sendTelemetry("status_response");
    } else {
      delay(5000);
    }
...
void loop() {
  ...
  // 1. AUTONOMNÍ KONTROLA ČASOVAČŮ A BEZPEČNOSTNÍCH LIMITŮ
  for (int i = 0; i < 8; i++) {
  ...
  // 2. TLAČÍTKA: KRÁTKÝ STISK vs 1.5s DLOUHÝ STISK PRO UKONČENÍ DELAY
  for (int i = 0; i < 4; i++) {
  ...
  // 3. PERIODICKÁ TELEMETRIE (Senzory + reálné stavy relé a odpočtů každých 5s)
