...
interface HardwareInfoViewProps {
  ...
  mode?: 'all' | 'esp32' | 'mqtt';
  isOffline?: boolean;
  onUpdateDeviceStatus?: (updated: ESP32DeviceStatus) => void;
}
...
export const HardwareInfoView: React.FC<HardwareInfoViewProps> = ({
  ...
  const sensorAlert = isOffline && (deviceStatus.sensorOfflineAlertEnabled ?? true);
        ...
        <>
          ...
          <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-4">
              <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/80">
                ...
                <span className="text-sm font-bold text-slate-800">8x OUT (kanály 1–8)</span>
              </div>
              <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/80">
          ...
          <div
            className={`rounded-2xl p-4 border transition-all duration-200 shadow-xs space-y-3 ${
              sensorAlert
                ? 'bg-rose-50/30 border-red-500 ring-2 ring-red-500/20'
          ...
          >
            <div className="flex items-center justify-between flex-wrap gap-2">
              <h3 className="font-bold text-slate-800 text-xs sm:text-sm flex items-center gap-1.5">
                ...
                <span>Senzorické vstupy (IN / OUT / Teploty)</span>
              </h3>

              {/* Status and offline alert toggle */}
              <div className="flex items-center gap-2">
                {isOffline && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold text-red-600 bg-red-100/80 px-2 py-0.5 rounded-full border border-red-300">
                ...
                {onUpdateDeviceStatus && (
                  <button
                    onClick={() =>
                      onUpdateDeviceStatus({
                        ...deviceStatus,
                        sensorOfflineAlertEnabled: !(deviceStatus.sensorOfflineAlertEnabled ?? true),
                      })
                    }
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full border transition ${
                      (deviceStatus.sensorOfflineAlertEnabled ?? true)
                        ? 'bg-red-50 text-red-700 border-red-300'
                        : 'bg-slate-100 text-slate-500 border-slate-200'
                    }`}
                    title="Aktivovat / deaktivovat optické offline zvýraznění pro senzory a vstupy"
                  >
                    Indikace offline: {(deviceStatus.sensorOfflineAlertEnabled ?? true) ? 'ZAP' : 'VYP'}
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div
                className={`p-3 rounded-xl flex items-center gap-3 border transition-all ${
                  sensorAlert
                    ? 'bg-rose-50/60 border-red-400/80'
              ...
              >
                ...
                <div>
                  ...
                  <span className="text-lg font-bold font-mono text-sky-800">
                    {isOffline ? '--.-' : (deviceStatus.oneWireTemp1?.toFixed(1) ?? '--.-')} °C
                  </span>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <span className="text-lg font-bold font-mono text-sky-800">
                    {isOffline ? '--.-' : (deviceStatus.oneWireTemp2?.toFixed(1) ?? '--.-')} °C
                  </span>
                </div>
              </div>
              ...
              {/* AM2320 Temp & Hum (I2C) */}
              <div
                className={`p-3 rounded-xl flex items-center gap-3 col-span-2 sm:col-span-1 border transition-all ${
                  sensorAlert
                    ? 'bg-rose-50/60 border-red-400/80'
              ...
              >
                ...
                <div>
                  ...
                  <span className="text-base font-bold font-mono text-amber-800">
                    {isOffline ? '---' : (deviceStatus.ldr ?? '---')} <span className="text-[10px]">RAW</span>
                  </span>
                </div>
              </div>
              ...
              {/* Inputs (Buttons) */}
              {[1, 2, 3, 4].map((inNum) => {
                const isActive = !isOffline && Boolean(deviceStatus[`input${inNum}Active` as keyof ESP32DeviceStatus]);
                return (
                  <div
                    key={inNum}
                  ...
                  >
                    ...
                    >
                      {isOffline ? 'NEZNÁMÝ' : isActive ? 'AKTIVNÍ' : 'Klid'}
                    </span>
                  </div>
                );
              })}
